...
<head>
    ...
    <link href="https://fonts.googleapis.com/css2?family=Acme&family=Irish+Grover&family=Josefin+Sans:ital,wght@0,100..700;1,100..700&family=Sawarabi+Gothic&display=swap" rel="stylesheet">
    <!-- モーダル追加のためのcss -->
...
<?php
$title = $row['title']; 
$colored_title = ''; // 色をつけた文字列を格納する変数
$colors = array('#ff6347', '#4682b4', '#9932cc', '#f4a460', '#2e8b57'); // 指定された色の配列
$color_index = 0; //初期値は０=今の色のインデックス

for ($i = 0; $i < strlen($title); $i++) {
    $colored_title .= '<span style="color:' . $colors[$color_index] . ';">' . $title[$i] . '</span>';
    $color_index = ($color_index + 1) % count($colors); // 次の色のインデックスを計算（割った余りによって指定）
}

echo '<header id="kids_header"><div class="kids_header"><p>' . $colored_title . '</p></div></header>';
?>

   <div class="kidsimg_wrap">
  ...
  <img class="modal-content" id="modal-img">
    ...
    <div class="tobtn"><a href="select.php" class="kids_btn">Back to List</a></div>
